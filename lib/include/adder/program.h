...
namespace adder {
  ...
  struct program_view_impl {
    ...
    symbol_entry_t * get_symbols() const {
      uint8_t const* table = ptr + get_header().symbol_table_offset;
      return (symbol_entry_t*)table;
    }

    symbol_entry_t * find_public_symbol(std::string_view const & find) const {
      for (uint64_t i = 0; i < get_header().symbol_count; ++i) {
        auto name = get_symbol(get_symbols()[i]);
        if (name == find) {
          return get_symbols() + i;
        }
      }
      return nullptr;
    }
  ...
  struct program {
    ...
    program_symbol_table_entry const* get_symbols() const {
