...
namespace adder {
  ...
  namespace vm {
    ...
    static InstructionMethod instruction_table[(uint8_t)op_code::count] = {
      [](machine * vm, instruction const * inst) { assert(inst->code == op_code::exit); vm; inst; },
      [](machine * vm, instruction const * inst) { assert(inst->code == op_code::noop); vm; inst; },
      [](machine * vm, instruction const * inst) { assert(inst->code == op_code::load); op::load(vm, inst->load); },
      [](machine * vm, instruction const * inst) { assert(inst->code == op_code::load_addr); op::load_addr(vm, inst->load_addr); },
      [](machine * vm, instruction const * inst) { assert(inst->code == op_code::load_offset );op::load_offset(vm, inst->load_offset); },
      [](machine * vm, instruction const * inst) { assert(inst->code == op_code::store); op::store(vm, inst->store); },
      [](machine * vm, instruction const * inst) { assert(inst->code == op_code::store_addr); op::store_addr(vm, inst->store_addr); },
      [](machine * vm, instruction const * inst) { assert(inst->code == op_code::store_offset); op::store_offset(vm, inst->store_offset); },
      [](machine * vm, instruction const * inst) { assert(inst->code == op_code::store_value); op::store_value(vm, inst->store_value); },
      ...
      [](machine * vm, instruction const * inst) { assert(inst->code == op_code::sub_i64); op::sub_i64(vm, inst->sub); },
      [](machine * vm, instruction const * inst) { assert(inst->code == op_code::sub_f64); op::sub_f64(vm, inst->sub); },
      [](machine * vm, instruction const * inst) { assert(inst->code == op_code::mul_i64); op::mul_i64(vm, inst->mul); },
      [](machine * vm, instruction const * inst) { assert(inst->code == op_code::mul_f64); op::mul_f64(vm, inst->mul); },
      [](machine * vm, instruction const * inst) { assert(inst->code == op_code::div_i64); op::div_i64(vm, inst->div); },
      [](machine * vm, instruction const * inst) { assert(inst->code == op_code::div_f64); op::div_f64(vm, inst->div); },
      [](machine * vm, instruction const * inst) { assert(inst->code == op_code::alloc_stack); op::alloc_stack(vm, inst->alloc_stack); },
      [](machine * vm, instruction const * inst) { assert(inst->code == op_code::free_stack); op::free_stack(vm, inst->free_stack); },
      [](machine * vm, instruction const * inst) { assert(inst->code == op_code::push); op::push(vm, inst->push); },
      [](machine * vm, instruction const * inst) { assert(inst->code == op_code::pop); op::pop(vm, inst->pop); },
      [](machine * vm, instruction const * inst) { assert(inst->code == op_code::jump); op::jump(vm, inst->jump); },
      [](machine * vm, instruction const * inst) { assert(inst->code == op_code::jump_relative); op::jump_relative(vm, inst->jump_relative); },
      [](machine * vm, instruction const * inst) { assert(inst->code == op_code::jump_if_zero_relative); op::jump_if_zero_relative(vm, inst->jump_if_zero_relative); },
      [](machine * vm, instruction const * inst) { assert(inst->code == op_code::jump_indirect); op::jump_indirect(vm, inst->jump_indirect); },
      [](machine * vm, instruction const * inst) { assert(inst->code == op_code::move); op::move(vm, inst->move); },
      [](machine * vm, instruction const * inst) { assert(inst->code == op_code::bitwise_and); op::bitwise_and(vm, inst->bitwise_op); },
      [](machine * vm, instruction const * inst) { assert(inst->code == op_code::bitwise_or); op::bitwise_or(vm, inst->bitwise_op); },
      [](machine * vm, instruction const * inst) { assert(inst->code == op_code::bitwise_xor); op::bitwise_xor(vm, inst->bitwise_op); },
      [](machine * vm, instruction const * inst) { assert(inst->code == op_code::bitwise_and_value); op::bitwise_and_value(vm, inst->bitwise_op_constant); },
      [](machine * vm, instruction const * inst) { assert(inst->code == op_code::bitwise_or_value); op::bitwise_or_value(vm, inst->bitwise_op_constant); },
      [](machine * vm, instruction const * inst) { assert(inst->code == op_code::bitwise_xor_value); op::bitwise_xor_value(vm, inst->bitwise_op_constant); },
      [](machine * vm, instruction const * inst) { assert(inst->code == op_code::set_non_zero); op::set_non_zero(vm, inst->set_non_zero); },
      [](machine * vm, instruction const * inst) { assert(inst->code == op_code::compare_i64); op::compare_i64(vm, inst->compare); },
      [](machine * vm, instruction const * inst) { assert(inst->code == op_code::compare_f64); op::compare_f64(vm, inst->compare); },
      [](machine * vm, instruction const * inst) { assert(inst->code == op_code::conditional_jump_relative); op::conditional_jump_relative(vm, inst->conditional_jump_relative); },
